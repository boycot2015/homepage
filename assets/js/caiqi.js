document.addEventListener('DOMContentLoaded', () => {
  const confettiEffect = (() => {
    ...
    function initializeButton() {
      const elements = document.querySelectorAll('.byt-aside-item.user');
      elements.forEach(el => {
    ...
    return {
      init: () => {
        if (!document.querySelector('.byt-aside-item.user > canvas')) return;
        setGlobals(".byt-aside-item.user > canvas");
        initializeButton();
