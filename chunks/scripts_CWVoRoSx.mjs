...
const get = () => {
  ...
  return query.find().then(function(data) {
    return data.sort((a, b) => a.attributes.order - b.attributes.order);
  }, function(err) {
    console.error("操作失败", err);
  });
};

const imgApi = "https://img.boycot.workers.dev";
const HitokotoApi = "https://v1.hitokoto.cn";
const musicApi = "https://api.i-meto.com/meting/api";
...
const customConfig = {
  ...
  "60sApi": "https://60s-api.viki.moe/v2",
  // https://api-v2.boycot.top/v2
  Api: "https://api.boycot.top/api"
  // https://api-v2.boycot.top/v2
};
const env = process.env.NODE_ENV;
if (env === "github") {
  const githubConfig = await import('./github.config_ChOs6Gc8.mjs');
...
const SITE_CONFIG = {
  ...
  // 作者头像
  Avatar: `/avatar.svg`,
  // https://cdn.jsdelivr.net/gh/boycot2015/picx-images-hosting@master/avatar/avatar.7pnvyzg2f.webp
  // 网站座右铭
  Motto: "越努力越幸运.",
  // Cover 网站缩略图
  ...
  HomeBanner: {
    ...
    PageHeight: "28.88rem",
    // 背景
    background: `url(${imgApi}) no-repeat center 60%/cover,url(${imgApi}?t=1696428800) no-repeat center 60%/cover,url(${imgApi}?t=1696428801) no-repeat center 60%/cover`
    // swiper 类型时生效，多张图片时逗号隔开，例如：'url1, url2, url3'
...
dayjs.locale("zh-cn");
const getDescription = (post, num = 150) => (post.rendered ? post.rendered.html.replace(/<[^>]+>/g, "").replace(/\s+/g, "") : post.body.replace(/\n/g, "").replace(/#/g, "")).slice(0, num) || "暂无简介";
const fmtTime = (time, fmt = "MMMM D, YYYY") => dayjs(time).utc().format(fmt);
const fmtPage = (page) => page?.replace(/\/|\./g, "") || null;
const getBeijingTime = (format) => {
  const beijing = (/* @__PURE__ */ new Date()).toLocaleString("en-US", {
...
const checkComment = () => {
  const CommentARR = Object.keys(SITE_CONFIG.Comment);
  const CommentItem = CommentARR.find((i) => SITE_CONFIG.Comment[i].enable);
  return CommentItem;
};

export { SITE_CONFIG as S, get as a, fmtPage as b, checkComment as c, getGreat as d, getBeijingTime as e, fmtTime as f, getDescription as g, imgApi as i };
